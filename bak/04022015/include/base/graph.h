...
namespace msd {
	namespace base {
		template <typename T> 
			class Graph {
				public:
					...
					//algorithms
					void single_source_dijkstra_full(T from, std::set<T>& nodes, 
				...
				private:
					struct heap_node_t {
						...
						heap_node_t(size_type _from_id, double _dist) : 
							from_id(_from_id), dist(_dist) {};
		 ...
		 */
		template <typename T>
			void Graph<T>::single_source_dijkstra_full(T from, std::set<T>& nodes, 
					std::map<T, std::vector<T> >& paths, std::map<T, double>& dist,
					std::map<T, double>& sigma)
			{
				//initialize
				...
				size_type from_id = _m_T_to_id[from];
				sigma_id[from_id] = 1.0f;

				pq.push(*(new heap_node_t(from_id, 0.0f)));
				...
				while (!pq.empty()) {
					...
					size_type u = node.from_id;

					if (seen.find(u))//already in
						continue;
					...
					for (size_type i = 0; i < _m_graph[u].size(); ++i) {
						...
						if (seen.find(v) == seen.end() && 
							...
							pq.push(*(new heap_node_t(v, uv_dist)));	
